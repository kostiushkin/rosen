...
%
% $Id: gen_activity.erl,v 1.13 2008/10/21 21:36:35 aristidena Exp $
%
...
%% Func: step/3
%%====================================================================
%% @spec step(Pid, ObjectState,TimeParams) -> Reply
%%       Pid = pid()
%%       ObjectState = object3d()
%%       TimeParams  = {Time,Delta}
%%       Time = float()
%%       Delta = float()
%%       Reply = term()
...
%%
%%
step (Pid, ObjectState,TimeParams) ->
  %%io:format ("step ~p~n", [Pid]),
  gen_server:call (Pid, {step, ObjectState,TimeParams}).
...
handle_call ({step, ObjectState,{Time,Delta}= TimeParams}, _, State) ->
  ...
  {reply, ok,
   State#activity_state {module_state = NewState}}.
