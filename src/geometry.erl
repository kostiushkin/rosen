...
%
% $Id: geometry.erl,v 1.12 2008/10/21 21:57:58 aristidena Exp $
%%
...
%% Function: cos/2
%%====================================================================
%% @spec cos(V1::vector(), V2::vector()) -> number()
%%
%% @doc Determines the cosine of the angle formed by two vectors (in
%%      the plane that contains both vectors).
%%
cos (V1 = #vector {}, V2 = #vector {}) ->
  C = dot (V1, V2) / (norm (V1) * norm (V2)),
  cos_validate (C).

%% cos_validate/1
cos_validate (C) when C > 1.0 -> 1.0;
%%
cos_validate (C) when C < -1.0 -> -1.0;
%%
cos_validate (C) -> C.
...
%% Func: centroid/1
%%====================================================================
%% @spec centroid([vector()]) -> vector()
%%
%% @doc Computes the centroid (or barycenter) of the finite set of
%%      points specified.
%%
centroid (Points = [H = #vector{} | T]) ->
  Sum = lists:foldl (fun (Elem = #vector{},
                          AccIn = #vector{}) ->
                             add (Elem, AccIn)
                     end,
                     _Acc0 = H,
