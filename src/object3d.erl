...
%
% $Id: object3d.erl,v 1.15 2008/10/21 21:55:05 aristidena Exp $
%
%%
...
%%====================================================================
%% Func: prev_position/2
%% @private
%%====================================================================
%% @spec prev_position(ObjPid::pid(), Position::vector()) -> ok
%%
%% @doc Sets the <i>(x,y,z)</i> previous position of an object.
%%
prev_position (Pid, Pos) ->
  gen_server:call (Pid, {set_prev_position, Pos}).
...
%% @private
%%====================================================================
%% @spec prev_axis(ObjPid::pid(), NewAxis::vector()) -> ok
%%
%% @doc Changes the previous axis of an object.
%%
prev_axis (Pid, NewAxis) ->
  gen_server:call (Pid, {set_prev_axis, NewAxis}).
...
%% @private
%%====================================================================
%% @spec prev_up(ObjPid::pid(), NewUp::vector()) -> ok
%%
%% @doc Changes the previous up direction of an object.
%%
prev_up (Pid, NewAxis) ->
  gen_server:call (Pid, {set_prev_up, NewAxis}).


%%====================================================================
%% Func: collision_points/1
%%====================================================================
%% @spec collision_points(ObjPid::pid()) -> [vector()]
%%
%% @doc Gets the points where the object specified is colliding.
%%
collision_points (Pid) ->
  gen_server:call (Pid, {get_collision_points}).


%%====================================================================
%% Func: collision_points/2
%%====================================================================
%% @spec collision_points(ObjPid::pid(), [vector()]) -> ok
%%
%% @doc Sets the points where the object specified is colliding.
%%
collision_points (Pid, Pos) ->
  gen_server:call (Pid, {set_collision_points, Pos}).

%%====================================================================
%% Func: set_Color/2
%%====================================================================
%% @spec set_Color(ObjPid::pid(), [vector()]) -> ok
%%
%% @doc Sets the color where ObjPid is the pid of the object specified.
%%
set_Color (Pid, Color) ->
  gen_server:call (Pid, {set_Color, Color}).


%%====================================================================
%% Func: set_Visible/2
%%====================================================================
%% @spec set_Visible(ObjPid::pid(), Bool()) -> ok
%%
%% @doc Sets the visibility property of an object.
%%
set_Visible (Pid, Bool) ->
  gen_server:call (Pid, {set_Visible, Bool}).
...
handle_call ({set_position, Pos}, _, State) ->
  ...
  {reply, ok, State#object3d_state { module_state = NewState }};
%%
handle_call ({set_prev_position, Pos}, _, State) ->
  ModuleState = State#object3d_state.module_state,
  NewState = ModuleState#object3d { prev_position = Pos },
  {reply, ok, State#object3d_state { module_state = NewState }};
%%
handle_call ({set_Color, Color}, _, State) ->
  ModuleState = State#object3d_state.module_state,
  NewState = ModuleState#object3d { color = Color },
  {reply, ok, State#object3d_state { module_state = NewState }};
%%
handle_call ({set_Visible, Bool}, _, State) ->
  ModuleState = State#object3d_state.module_state,
  NewState = ModuleState#object3d { visible = Bool },
  {reply, ok, State#object3d_state { module_state = NewState }};
%%
...
handle_call ({set_axis, NewAxis}, _, State) ->
  ...
  {reply, ok, State#object3d_state { module_state = NewState }};
%%
handle_call ({set_prev_axis, NewAxis}, _, State) ->
  ModuleState = State#object3d_state.module_state,
  NewState = ModuleState#object3d { prev_axis = NewAxis },
  {reply, ok, State#object3d_state { module_state = NewState }};
%%
...
handle_call ({set_up, NewAxis}, _, State) ->
  ...
  {reply, ok, State#object3d_state { module_state = NewState }};
%%
handle_call ({set_prev_up, NewAxis}, _, State) ->
  ModuleState = State#object3d_state.module_state,
  NewState = ModuleState#object3d { up = NewAxis },
  {reply, ok, State#object3d_state { module_state = NewState }};
%%
...
%%
handle_call ({get_collision_points}, _, State) ->
  ModuleState = State#object3d_state.module_state,
  CollisionPoints = ModuleState#object3d.collision_points,
  {reply, CollisionPoints, State};
%%
handle_call ({set_collision_points, CollisionPoints}, _, State) ->
  ModuleState = State#object3d_state.module_state,
  NewModuleState = ModuleState#object3d { collision_points = CollisionPoints },
  {reply, ok, State#object3d_state { module_state = NewModuleState }}.
...
init ([Module, Params]) ->
  ...
  {ok, NewModuleState} = collision_related_init (Params, ModuleState),
...
%% collision_related_init/2
collision_related_init (_UserInput = #object3d{ relative_meshes = UM },
                        ModuleState = #object3d{ relative_meshes = M,
                                                 position = P,
                                                 axis = A,
                                                 up = U }) ->
  %% if the user specified a custom mesh, obey; otherwise, if the
  %% module of the object ignored the creation of the mesh, consider
  %% an empty mesh
...
RelativeMeshes = set_Meshes(UM,M),


  %% in this version of ROSEN, storing the mesh as trimesh
  %%
  %% set previous position, axis and up to current ones
  {ok,
   ModuleState#object3d { relative_meshes = trimesh:from_mesh (RelativeMeshes),
                          prev_position = P,
                          prev_axis = A,
                          prev_up = U }}.
...
%
set_Meshes(undefined,undefined)-> [];
set_Meshes(undefined,M)-> M;
set_Meshes(UM,_)-> UM.
