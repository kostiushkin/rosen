...
%
% $Id: robot_path.erl,v 1.7 2008/10/21 21:36:35 aristidena Exp $
%
...
%  io:format("Forward ~p ~p~n",[PathState#path_state.path_tail,PathState]),
  ...
  {RobotX, RobotY, _} = robot:command (PathState#path_state.robot_pid,
                                       {get_position}),
%io:format("Rx ~p RY ~p~n",[RobotX,RobotY]),
  D = abs (geometry:distance (SPos, {RobotX, RobotY}) - abs(X)),

 io:format ("Robot ~p D ~p, Motion ~p  Control ~p ~n", [PathState#path_state.robot_pid,D,PathState#path_state.motion_tolerance, D < PathState#path_state.motion_tolerance]),
  ...
  if
    ...
    true ->
	  robot:command (PathState#path_state.robot_pid,
			 {set_speed, sgn(X) * PathState#path_state.motion_speed, 0.0}),
...
step (ObjectPid, ObjectState, _, _,
 ...
 D = geometry:normalize_angle (X - (CPos - SPos)),
  DA = abs (D),
  io:format ("Robot ~p DA ~p,ROT ~p Diff ~p~n", [PathState#path_state.robot_pid,DA,PathState#path_state.rotation_tolerance,DA < PathState#path_state.rotation_tolerance]),
  if
...
%% Func: set_property/4
%%====================================================================
set_property (_, path, Value, PathState) ->
  PathState#path_state { path = Value,
			 current_target = nil,
			 path_tail = [],
                         restart = true}.
